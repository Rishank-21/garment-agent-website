...
export default function WhiteLabelingPage() {
  return <div className="min-h-screen bg-[#161612] text-[#F4EFE6]"><main>
    <EditorialPageIntro eyebrow="05 / White Labeling" title={<>Your brand.<br /><span className="text-transparent stroke-text">Your vision.</span><br />Our expertise.</>} description="From concept to finished garment, we support businesses looking to create their own apparel collections through a relevant sourcing and manufacturing network." image="https://images.unsplash.com/photo-1558769132-cb1aea458c5e?q=80&w=1400" imageAlt="White labeling apparel development" ctaLabel="Start your white labeling" ctaHref="/#enquiry" />
    <section className="paper-surface px-5 py-20 sm:px-8 lg:px-12 lg:py-28"><div className="mx-auto max-w-[1500px]"><div className="flex flex-col justify-between gap-6 border-b border-[#161612]/15 pb-8 lg:flex-row lg:items-end"><div><span className="mono-label text-[10px] text-[#C95A1A]">Make your own brand</span><div className="accent-rule mt-5" /><h2 className="mt-7 max-w-4xl font-display text-4xl font-black uppercase leading-[.86] tracking-[-.08em] sm:text-7xl">A collection is more<br />than a product list.</h2></div><p className="max-w-sm text-sm leading-relaxed text-[#161612]/65">Build a focused range of cotton pants, shirts, T-shirts, ladies wear, kids wear or a custom garment collection around your customer and market.</p></div><div className="mt-14 grid gap-px bg-[#161612]/15 md:grid-cols-2 lg:grid-cols-4">{steps.map(({ step, title, desc, icon: Icon }) => <article key={step} className="bg-[#F4EFE6] p-7 transition-colors hover:bg-[#E7E0D3]"><div className="flex items-center justify-between"><Icon size={22} className="text-[#C95A1A]" /><span className="font-mono text-[10px] text-[#161612]/45">{step}</span></div><h3 className="mt-16 font-display text-2xl font-black uppercase leading-[.9] tracking-[-.06em]">{title}</h3><p className="mt-4 text-sm leading-relaxed text-[#161612]/65">{desc}</p></article>)}</div></div></section>
    <section className="bg-[#1E211E] px-5 py-20 sm:px-8 lg:px-12 lg:py-24"><div className="mx-auto grid max-w-[1500px] gap-10 lg:grid-cols-2"><h2 className="font-display text-3xl font-black uppercase leading-[.86] tracking-[-.07em] sm:text-6xl">Custom design.<br />Fabric sourcing.<br />Production support.</h2><div className="border-t border-[#F4EFE6]/15 pt-5 text-sm leading-relaxed text-[#F4EFE6]/65"><p>White labeling is a working process. The final scope depends on the garment category, fabric, quantity, sampling requirements, branding details and delivery plan.</p><p className="mt-5 text-[#F4EFE6]/85">Bring your vision and we will help map the next practical step.</p></div></div></section>
