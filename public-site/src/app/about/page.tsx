      ...
      <main>
        ...
        <section className="paper-surface px-5 py-20 sm:px-8 lg:px-12 lg:py-28">
          <div className="mx-auto grid max-w-[1440px] gap-12 lg:grid-cols-[.9fr_1.1fr] lg:items-start">
            <div>
              <span className="mono-label text-[10px] text-[#f05a24]">Story / not a slogan</span>
            ...
            <div className="space-y-5 text-sm leading-relaxed text-[#151613]/70 sm:text-base">
              ...
              <div className="grid gap-3 border-t border-[#151613]/15 pt-5 sm:grid-cols-3">
                <div><p className="font-display text-3xl text-[#f05a24]">02</p><p className="mono-label mt-2 text-[9px] text-[#151613]/50">Generations</p></div>
                <div><p className="font-display text-3xl text-[#f05a24]">01</p><p className="mono-label mt-2 text-[9px] text-[#151613]/50">Trusted vision</p></div>
                <div><p className="font-display text-3xl text-[#f05a24]">∞</p><p className="mono-label mt-2 text-[9px] text-[#151613]/50">Relationships</p></div>
              </div>
            </div>
          </div>
        </section>

        <section className="bg-[#1e201c] px-5 py-20 sm:px-8 lg:px-12 lg:py-28">
          <div className="mx-auto max-w-[1440px]">
            <div className="mb-10 flex items-end justify-between border-b border-[#f7f2e9]/15 pb-5">
              <div><span className="mono-label text-[10px] text-[#ffb800]">Capabilities / factory highlights</span><h2 className="mt-4 font-display text-5xl font-black uppercase leading-[.86] tracking-[-.08em] sm:text-7xl">Verify the detail.<br />Keep the vision.</h2></div>
              <Building2 size={26} className="hidden text-[#f05a24] sm:block" />
            </div>
            <div className="grid gap-px bg-[#f7f2e9]/12 sm:grid-cols-2">
              {capabilities.map(({ icon: Icon, title, text }) => (
                <article key={title} className="bg-[#151613] p-7 transition-colors hover:bg-[#23251f] sm:p-9"><Icon size={20} className="text-[#f05a24]" /><h3 className="mt-16 font-display text-3xl font-black uppercase leading-[.9] tracking-[-.07em]">{title}</h3><p className="mt-4 max-w-sm text-sm leading-relaxed text-[#f7f2e9]/60">{text}</p></article>
              ))}
            </div>
          </div>
        </section>

        <section className="paper-surface border-y border-[#151613]/10 px-5 py-16 sm:px-8 lg:px-12 lg:py-24">
          <div className="mx-auto grid max-w-[1440px] gap-8 lg:grid-cols-[.7fr_1.3fr]">
            <div><span className="mono-label text-[10px] text-[#f05a24]">Certifications status</span><h2 className="mt-4 font-display text-4xl font-black uppercase leading-[.88] tracking-[-.08em] sm:text-5xl">Evidence belongs in the buying file.</h2></div>
            <div className="grid gap-px bg-[#151613]/15 sm:grid-cols-3">
              {[['Current status','Confirm with Himat Textile'],['Buyer action','Request the relevant current certificates for your product and market.'],['Documentation','Verified files can be shared as part of the sourcing and compliance discussion.']].map(([label, text]) => <div key={label} className="bg-[#f7f2e9] p-6"><p className="mono-label text-[10px] text-[#151613]/45">{label}</p><p className="mt-8 font-display text-xl font-black uppercase leading-[.9] tracking-[-.06em]">{text}</p></div>)}
            </div>
          </div>
        </section>
        <HimatInquiry />
