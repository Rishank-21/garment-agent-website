...
export default function ExportPage() {
  return <div className="min-h-screen bg-[#161612] text-[#F4EFE6]"><main>
    ...
    <section className="paper-surface px-5 py-20 sm:px-8 lg:px-12 lg:py-28"><div className="mx-auto max-w-[1500px]"><span className="mono-label text-[10px] text-[#C95A1A]">Export support / practical readiness</span><div className="accent-rule mt-5" /><h2 className="mt-7 max-w-4xl font-display text-4xl font-black uppercase leading-[.86] tracking-[-.08em] sm:text-7xl">Clear details make<br />global buying easier.</h2><div className="mt-14 grid gap-px bg-[#161612]/15 sm:grid-cols-2">{support.map(({ title, desc, icon: Icon }, index) => <article key={title} className="bg-[#F4EFE6] p-8 transition-colors hover:bg-[#E7E0D3]"><div className="flex items-center justify-between"><Icon size={24} className="text-[#C95A1A]" /><span className="font-mono text-[10px] text-[#161612]/45">0{index + 1}</span></div><h3 className="mt-16 font-display text-2xl font-black uppercase leading-[.9] tracking-[-.06em]">{title}</h3><p className="mt-4 max-w-md text-sm leading-relaxed text-[#161612]/65">{desc}</p></article>)}</div></div></section>
    <section className="bg-[#1E211E] px-5 py-20 sm:px-8 lg:px-12 lg:py-24"><div className="mx-auto grid max-w-[1500px] gap-10 lg:grid-cols-[.8fr_1.2fr]"><h2 className="font-display text-3xl font-black uppercase leading-[.86] tracking-[-.07em] sm:text-6xl">Sourcing first.<br />Shipping next.</h2><div className="border-t border-[#F4EFE6]/15 pt-5 text-sm leading-relaxed text-[#F4EFE6]/65"><p>International orders benefit from a B2B product brief, realistic quantity planning and documented quality expectations. Specific export claims, certifications and logistics arrangements should be confirmed directly for each requirement.</p><p className="mt-5 text-[#F4EFE6]/85">Tell us the product, market, quantity and delivery context to start the right conversation.</p></div></div></section>
