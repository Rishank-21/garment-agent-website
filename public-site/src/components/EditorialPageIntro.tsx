...
export function EditorialPageIntro({ eyebrow, title, description, image, imageAlt, ctaLabel, ctaHref }: EditorialPageIntroProps) {
  return (
    <section className="relative overflow-hidden bg-[#151613] px-5 pb-20 pt-32 text-[#f7f2e9] sm:px-8 lg:px-12 lg:pb-28 lg:pt-40">
      {image && (
        <div className="absolute inset-y-0 right-0 hidden w-1/2 lg:block">
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-[#151613] via-[#151613]/75 to-[#151613]/10" />
        </div>
      )}
      <div className="noise-layer absolute inset-0" />
      <div className="relative mx-auto max-w-[1500px]">
        <div className="max-w-4xl">
          <span className="mono-label block text-[10px] tracking-[0.24em] text-[#ffb800]">{eyebrow}</span>
          <div className="mt-5 h-[3px] w-16 bg-gradient-to-r from-[#f05a24] to-[#ffb800]" />
          <h1 className="mt-7 font-display text-[clamp(3.5rem,8vw,7.5rem)] font-black uppercase leading-[0.86] tracking-[-0.07em]">{title}</h1>
          <p className="mt-8 max-w-xl text-sm leading-relaxed text-[#f7f2e9]/70 sm:text-base">{description}</p>
          {ctaLabel && ctaHref && (
